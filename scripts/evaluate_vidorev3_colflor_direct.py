...
def main() -> None:  # noqa: C901
    ...
    t0 = time.time()
    dataset_path = f"vidore/vidore_v3_{args.domain}"
    ...
    for row in corpus:
        if args.limit_corpus and seen >= args.limit_corpus:
            break
        ...
        if len(images) >= args.image_batch_size:
            flush()
            pbar.update(args.image_batch_size)
    flush()
    pbar.close()
