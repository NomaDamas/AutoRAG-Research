...
class TestInferSubcategory:
    """Tests for _infer_subcategory()."""

    def test_retrieval_from_target_class(self):
        """Infers 'retrieval' when _target_ contains 'Retrieval'."""
        from autorag_research.plugin_registry import _infer_subcategory

        cfg = {"_target_": "my_plugin.pipeline.MySearchRetrievalPipelineConfig"}
        assert _infer_subcategory(cfg) == "retrieval"

    def test_retrieval_from_target_module(self):
        """Infers 'retrieval' when _target_ module path contains '.retrieval.'."""
        from autorag_research.plugin_registry import _infer_subcategory

        cfg = {"_target_": "my_plugin.retrieval.pipeline.SearchConfig"}
        assert _infer_subcategory(cfg) == "retrieval"

    def test_generation_from_target_class(self):
        """Infers 'generation' when _target_ contains 'Generation'."""
        from autorag_research.plugin_registry import _infer_subcategory

        cfg = {"_target_": "my_plugin.pipeline.MyGenGenerationPipelineConfig"}
        assert _infer_subcategory(cfg) == "generation"

    def test_generation_from_target_module(self):
        """Infers 'generation' when _target_ module path contains '.generation.'."""
        from autorag_research.plugin_registry import _infer_subcategory

        cfg = {"_target_": "my_plugin.generation.pipeline.GenConfig"}
        assert _infer_subcategory(cfg) == "generation"

    def test_no_target_returns_none(self):
        """Returns None when _target_ is missing."""
        from autorag_research.plugin_registry import _infer_subcategory

        cfg = {"description": "Some plugin"}
        assert _infer_subcategory(cfg) is None

    def test_ambiguous_target_returns_none(self):
        """Returns None when _target_ doesn't match retrieval or generation."""
        from autorag_research.plugin_registry import _infer_subcategory

        cfg = {"_target_": "my_plugin.pipeline.SomethingConfig"}
        assert _infer_subcategory(cfg) is None

    def test_flat_layout_with_retrieval_target_gets_subcategory(self, tmp_path):
