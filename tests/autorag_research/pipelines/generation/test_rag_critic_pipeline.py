...
def create_lightweight_rag_critic_pipeline(
    *,
    ...
    critic_llm: MagicMock | None = None,
    ...
    from autorag_research.pipelines.generation.rag_critic import (
        DEFAULT_AGENT_DECOMPOSE_PROMPT,
        DEFAULT_AGENT_GENERATE_ANSWER_PROMPT,
        DEFAULT_AGENT_REFINE_EXPLAIN_PROMPT,
        DEFAULT_AGENT_REFINE_SUMMARIZE_PROMPT,
        DEFAULT_AGENT_REWRITE_CLARIFY_PROMPT,
        DEFAULT_AGENT_REWRITE_CUSTOM_PROMPT,
        DEFAULT_AGENT_REWRITE_EXPAND_PROMPT,
        ...
        DEFAULT_TRAINED_CRITIC_PROMPT,
        RAGCriticPipeline,
    )

    pipeline = object.__new__(RAGCriticPipeline)
    base_llm = llm or MagicMock()
    pipeline._answer_prompt_template = DEFAULT_ANSWER_PROMPT
    pipeline._critic_prompt_template = DEFAULT_CRITIC_PROMPT
    pipeline._trained_critic_prompt_template = DEFAULT_TRAINED_CRITIC_PROMPT
    pipeline._planner_prompt_template = DEFAULT_PLANNER_PROMPT
    pipeline._agent_system_prompt_template = DEFAULT_AGENT_SYSTEM_PROMPT
    pipeline._agent_user_prompt_template = DEFAULT_AGENT_USER_PROMPT
    ...
    pipeline._agent_generate_answer_prompt_template = DEFAULT_AGENT_GENERATE_ANSWER_PROMPT
    pipeline._agent_rewrite_clarify_prompt_template = DEFAULT_AGENT_REWRITE_CLARIFY_PROMPT
    pipeline._agent_rewrite_expand_prompt_template = DEFAULT_AGENT_REWRITE_EXPAND_PROMPT
    pipeline._agent_rewrite_custom_prompt_template = DEFAULT_AGENT_REWRITE_CUSTOM_PROMPT
    pipeline._agent_decompose_prompt_template = DEFAULT_AGENT_DECOMPOSE_PROMPT
    pipeline._agent_refine_summarize_prompt_template = DEFAULT_AGENT_REFINE_SUMMARIZE_PROMPT
    pipeline._agent_refine_explain_prompt_template = DEFAULT_AGENT_REFINE_EXPLAIN_PROMPT
    pipeline._critic_output_format = "json_actions"
    pipeline._planner_output_format = "json_actions"
    pipeline._max_iterations = max_iterations
    pipeline._max_actions_per_iteration = max_actions_per_iteration
    pipeline._llm = base_llm
    pipeline._critic_llm = critic_llm or base_llm
...
class TestRAGCriticPipelineUnit:
    ...
    def test_planner_prompt_requires_integer_top_k_literals(self):
        """Planner prompt should explicitly require JSON integer literals for top_k."""
        from autorag_research.pipelines.generation.rag_critic import DEFAULT_PLANNER_PROMPT

        assert "emit a JSON integer literal" in DEFAULT_PLANNER_PROMPT
        assert "Do not emit top_k as a string" in DEFAULT_PLANNER_PROMPT
    ...
    def test_parse_json_payload_handles_python_style_dict_output(self):
        """Published critic checkpoints may emit Python-literal dict strings."""
        from autorag_research.pipelines.generation.rag_critic import RAGCriticPipeline

        payload = RAGCriticPipeline._parse_json_payload("{'Judgement': 'Correct'}")

        assert payload == {"Judgement": "Correct"}
    ...
    def test_map_trained_critic_tags_to_actions(self):
        """Published RAG-Critic tags should map to local corrective actions."""
        from autorag_research.pipelines.generation.rag_critic import RAGCriticPipeline

        actions = RAGCriticPipeline._map_trained_critic_tags_to_actions([
            "Entity and Concept Confusion",
            "Insufficient or Incomplete Information Retrieval",
        ])

        assert actions == ["rewrite_query", "decompose_query", "retrieval"]
    ...
    @pytest.mark.asyncio
    async def test_run_trained_critic_maps_error_tags_to_actions(self):
        """Published RAG-Critic output should be adapted into local verdict/actions."""
        from autorag_research.util import TokenUsageTracker

        critic_llm = MagicMock()
        response = MagicMock()
        response.content = (
            "{'Judgement': 'Error', 'Error_analysis': 'Need better retrieval and query focus.', "
            "'tag1': ['Incomplete Information'], "
            "'tag2': ['Insufficient or Incomplete Information Retrieval', 'Entity and Concept Confusion'], "
            "'tag3': ['Information Retrieval Failure']}"
        )
        response.usage_metadata = {"input_tokens": 7, "output_tokens": 3, "total_tokens": 10}
        response.response_metadata = {}
        critic_llm.ainvoke = AsyncMock(return_value=response)

        pipeline = create_lightweight_rag_critic_pipeline(critic_llm=critic_llm)
        pipeline._critic_output_format = "rag_critic_tags"
        ...
        assert critique["feedback"] == "Need better retrieval and query focus."
        assert critique["recommended_actions"] == ["retrieval", "rewrite_query", "decompose_query"]
        assert critique["tag2"] == [
            "Insufficient or Incomplete Information Retrieval",
            "Entity and Concept Confusion",
        ]

    @pytest.mark.asyncio
    async def test_run_trained_critic_maps_correct_judgement_to_approval(self):
        """Published RAG-Critic correct judgements should short-circuit approval."""
        from autorag_research.util import TokenUsageTracker

        critic_llm = MagicMock()
        response = MagicMock()
        response.content = "{'Judgement': 'Correct'}"
        response.usage_metadata = {"input_tokens": 4, "output_tokens": 1, "total_tokens": 5}
        response.response_metadata = {}
        critic_llm.ainvoke = AsyncMock(return_value=response)
        ...
        assert critique["recommended_actions"] == []

    @pytest.mark.asyncio
    async def test_plan_actions_uses_official_python_agent_prompt_mode(self):
        """Official-style mode should ask the planner for Python function-call code."""
        from autorag_research.util import TokenUsageTracker

        pipeline = create_lightweight_rag_critic_pipeline()
        pipeline._planner_output_format = "python_agent"
        pipeline._invoke_and_record = AsyncMock(return_value="final_answer = GenerateAnswer(question, doc_list)")  # type: ignore[method-assign]

        actions = await pipeline._plan_actions(
            "What is RAG-Critic?",
            ["chunk 1", "chunk 2"],
            "Initial answer",
            {"feedback": "Need better grounding", "tag2": ["Data Insufficiency in Retrieval"]},
            TokenUsageTracker(),
        )

        assert actions == [{"action": "agent_code", "code": "final_answer = GenerateAnswer(question, doc_list)"}]

    @pytest.mark.asyncio
    async def test_execute_agent_code_plan_runs_official_style_function_calls(self):
        """Official-style planner code should execute through the restricted evaluator."""
        from autorag_research.util import TokenUsageTracker
        ...
        async def mock_ainvoke(prompt: str):
            response = MagicMock()
            if "rewritten query" in prompt.lower():
                response.content = '{"query": "clarified question"}'
            elif "smaller, more specific subqueries" in prompt.lower():
                response.content = '["subquestion 1", "subquestion 2"]'
            elif "refined_document" in prompt.lower():
                response.content = '{"refined_document": "refined chunk"}'
            else:
                response.content = "So the final answer is: grounded answer"
            response.usage_metadata = {"input_tokens": 5, "output_tokens": 2, "total_tokens": 7}
            response.response_metadata = {}
            return response

        llm.ainvoke = AsyncMock(side_effect=mock_ainvoke)
        retrieval_pipeline = create_mock_retrieval_pipeline(default_results=[{"doc_id": 7, "score": 0.91}])
        pipeline = create_lightweight_rag_critic_pipeline(llm=llm, retrieval_pipeline=retrieval_pipeline)

        answer, documents, working_context, answer_updated = await pipeline._execute_agent_code_plan(
            """
rewritten_queries = RewriteQuery(question, "clarify")
docs = Retrieval(rewritten_queries[0], 2)
refined_doc = RefineDoc(question, docs[0], "summarize")
final_answer = GenerateAnswer(question, [refined_doc], "be precise")
            """.strip(),
            original_query="What is RAG-Critic?",
            documents=[{"doc_id": 1, "score": 0.4, "content": "chunk 1"}],
            working_context="chunk 1",
        ...
        assert answer == "So the final answer is: grounded answer"
        assert answer_updated is True
        assert [document["doc_id"] for document in documents] == [7]
        assert "doc_id=7" in working_context

    def test_coerce_positive_int_falls_back_for_invalid_planner_values(self):
        """Planner-provided top_k should fall back when it is invalid."""
        from autorag_research.pipelines.generation.rag_critic import RAGCriticPipeline

        assert RAGCriticPipeline._coerce_positive_int("two", 3) == 3
        assert RAGCriticPipeline._coerce_positive_int(None, 3) == 3
        assert RAGCriticPipeline._coerce_positive_int(True, 3) == 3
        assert RAGCriticPipeline._coerce_positive_int(0, 3) == 1

    @pytest.mark.asyncio
    async def test_handle_retrieval_action_falls_back_to_default_top_k(self):
        """Invalid planner top_k should not crash retrieval and should use the default top_k."""
        pipeline = create_lightweight_rag_critic_pipeline()

        documents, context = await pipeline._handle_retrieval_action(
            {"action": "retrieval", "top_k": "two"},
            original_query="What is RAG-Critic?",
            working_query="What is RAG-Critic?",
            documents=[],
            rewritten_queries=[],
            sub_questions=[],
            retrieval_history=[],
        ...
        pipeline._retrieval_pipeline.retrieve.assert_awaited_once_with("What is RAG-Critic?", 3)
        assert [document["doc_id"] for document in documents] == [1, 2, 3]
        assert "doc_id=1" in context

    @pytest.mark.asyncio
    async def test_rewrite_query_falls_back_to_existing_query_when_blank(self):
        """Blank rewrite outputs should preserve the current working query."""
        from autorag_research.util import TokenUsageTracker

        pipeline = create_lightweight_rag_critic_pipeline()
        pipeline._invoke_and_record = AsyncMock(return_value="   ")  # type: ignore[method-assign]

        rewritten = await pipeline._rewrite_query(
            "original query",
            "Need a clearer retrieval query",
            "rewrite it",
            TokenUsageTracker(),
        )
