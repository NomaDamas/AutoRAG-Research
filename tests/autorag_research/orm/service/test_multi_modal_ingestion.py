...
class TestGetOrCreateFiles:
    def test_get_or_create_files_create_new(self, multi_modal_service: MultiModalIngestionService, db_session: Session):
        files_data = [("/test/new_file_1.png", "image"), ("/test/new_file_2.png", "image")]
        results = multi_modal_service.get_or_create_files(files_data)

        assert len(results) == 2
        assert all(was_created for _, was_created in results)
        assert all(file_id is not None for file_id, _ in results)

        for file_id, _ in results:
            db_session.delete(db_session.get(File, file_id))
        db_session.commit()

    def test_get_or_create_files_get_existing(self, multi_modal_service: MultiModalIngestionService):
        results = multi_modal_service.get_or_create_files([("/data/img1.png", "image")])

        assert len(results) == 1
        file_id, was_created = results[0]
        assert not was_created
        assert file_id == 6

    def test_get_or_create_files_mixed(self, multi_modal_service: MultiModalIngestionService, db_session: Session):
        files_data = [("/data/img1.png", "image"), ("/test/brand_new_file.png", "image")]
        results = multi_modal_service.get_or_create_files(files_data)

        assert len(results) == 2
        existing_file_id, existing_created = results[0]
        new_file_id, new_created = results[1]

        assert not existing_created
        assert existing_file_id == 6
        assert new_created
        assert new_file_id is not None

        db_session.delete(db_session.get(File, new_file_id))
        db_session.commit()
