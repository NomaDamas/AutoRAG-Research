...
def beir_db_engine(beir_ingestor):
    ...
    pkey_type = beir_ingestor.detect_primary_key_type()
    schema = create_schema(EMBEDDING_DIM, pkey_type)
...
@pytest.fixture
def service(session_factory_beir, beir_ingestor):
    service = TextDataIngestionService(session_factory_beir)
    beir_ingestor.set_service(service)
    yield service


@pytest.fixture(scope="session")
def beir_ingestor(db_session):
    ingestor = BEIRIngestor(MockEmbedding(EMBEDDING_DIM), "scifact")
    yield ingestor
