...
class File(Base):
    ...
    type: Mapped[str] = mapped_column(String(255), nullable=False)  # raw, image, audio, video
...
class Page(Base):
    ...
    image_path: Mapped[int | None] = mapped_column(BigInteger, ForeignKey("file.id", ondelete="SET NULL"), unique=True)
...
class CaptionChunkRelation(Base):
    ...
    caption_id: Mapped[int] = mapped_column(BigInteger, ForeignKey("caption.id", ondelete="CASCADE"), primary_key=True)
